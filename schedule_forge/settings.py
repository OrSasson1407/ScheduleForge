...
SORT_CRITERIA_TITLES = {
    ...
    "min_gap_between_moeds":
        "3.6 maximise the gap between moed Aleph and moed Bet of the same course",
    "worst_window_count":
        "3.7 minimise the most exams of a program and year in any date window",
}
...
#: The five criteria of version 3.0, most protective first - the default when
#: the settings do not say otherwise, so that a plain run already looks for
#: the best systems instead of merely the first ones that happen to pass the
#: thresholds. `min_gap_between_moeds` (3.6) and `worst_window_count` (3.7)
#: are deliberately left out of the default: both are new, off-by-default
#: features, and adding either to every run's sort order by default would
#: change tie-breaking for runs that never asked for it, breaking the "unset
#: setting = today's exact behaviour" rule every other optional feature in
#: this project follows.
DEFAULT_SORT_CRITERIA = SORT_CRITERIA[:5]
...
class SchedulingSettings(object):
    ...
    __slots__ = ("min_days_between_obligatory", "min_days_between_any",
                 ...
                 "max_exams_per_window", "window_days", "time_slots",
                 "require_rooms", "sort_criteria", "max_candidates",
                 "max_examined", "time_limit_seconds", "default_students")
    ...
    def __init__(self, min_days_between_obligatory=None, min_days_between_any=None,
                 ...
                 max_exams_per_day=None, min_gap_between_moeds=None,
                 max_exams_per_window=None, window_days=None, time_slots=None,
                 require_rooms=False, sort_criteria=None,
        ...
        #: 2.7 - exams of one program and year inside any `window_days` span.
        #: Both must be set together, or neither - see `validate`.
        self.max_exams_per_window = max_exams_per_window
        self.window_days = window_days
        #: Item 2 - the times of day an exam may start, e.g. ["09:00", "13:00"];
        #: unset (the default) leaves time of day out of the search entirely,
        #: exactly as it always has been.
        self.time_slots = list(time_slots) if time_slots else None
    ...
    def validate(self):
        for name in ("min_days_between_obligatory", "min_days_between_any",
                     ...
                     "min_gap_between_moeds", "max_exams_per_window",
                     "window_days"):
        ...
        if (self.max_exams_per_window is None) != (self.window_days is None):
            raise SettingsError(
                "max_exams_per_window and window_days must be set together")
        if self.time_slots is not None:
            if not all(re.match(r"^\d{1,2}:\d{2}$", slot) for slot in self.time_slots):
                raise SettingsError("every time slot must look like 'HH:MM'")
            if len(set(self.time_slots)) != len(self.time_slots):
                raise SettingsError("a time slot is named twice")
    ...
    def has_thresholds(self):
        return any((self.min_days_between_obligatory, self.min_days_between_any,
                    ...
                    self.min_gap_between_moeds, self.max_exams_per_window,
                    self.time_slots, self.require_rooms))
    ...
    def pairwise_thresholds_only(self):
        ...
        return not any((self.max_elective_collisions is not None,
                        self.min_obligatory_span, self.max_exams_per_day,
                        self.max_exams_per_window, self.time_slots,
                        self.require_rooms))
    ...
    def describe_thresholds(self):
        ...
        if self.min_gap_between_moeds:
            lines.append("2.6 at least %d days between moed Aleph and moed Bet "
                         "of the same course" % self.min_gap_between_moeds)
        if self.max_exams_per_window:
            lines.append("2.7 at most %d exams of a program and year in any "
                         "%d day window" % (self.max_exams_per_window, self.window_days))
        if self.time_slots:
            lines.append("every exam is assigned one of %d time slots, with no "
                         "two conflicting exams sharing one on the same date"
                         % len(self.time_slots))
